...
class NewsViewModel @Inject constructor(
        ...
        when (event) {
            ...
            is NewsEvent.OnClickInitialize -> {
                intent {
                    copy(
                        subjectDialogToken = null,
                        subjects = this.subjects.map { it.copy(isChecked = true) }.toPersistentList(),
                        filteredNews = getFilteredList(subjects = this.subjects.map { it.copy(isChecked = true) }).toPersistentList()
                    )
                }
            }
            ...
            is NewsEvent.OnClickNewNews -> {
                ...
                postSideEffect(NewsSideEffect.ScrollUp)
            }
        }
    }
     ...
     */
    private fun getFilteredList(news: List<News>? = null, category: ChipState? = null, subjects: List<ChipState>? = null): List<News> {
        val notNullNewsList: MutableList<News> = news?.toMutableList() ?: currentState.news.toMutableList()
